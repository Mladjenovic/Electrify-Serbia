...
@Component({
  selector: 'app-safety-document-multimedia',
  templateUrl: './safety-document-multimedia.component.html',
  styleUrls: ['./safety-document-multimedia.component.scss'],
})
export class SafetyDocumentMultimediaComponent implements OnInit {
  files: File[] = [];
  id: number = 0;
  p: number = 0;
  url: string = '';
    ...
    for (var item of this.files) {
      ...
      reader.onload = (event: any) => {
        ...
        this.service.postDataMultimedia(multimedia).subscribe((res) => {
          ...
          this.toastr.success('Succesfuly uploaded', 'Dociment Info');
        });
      };
    }
